...
uint32_t fdt_u32_le2be (const void *addr)  // 4 bytes one take
{
	// uart_send_string("in fdt_u32_le2be\n");
	...
	// uart_send_string("out fdt_u32_le2be\n");
	return ret;
}
...
int parse_struct (fdt_callback cb, uintptr_t cur_ptr, uintptr_t strings_ptr,uint32_t totalsize) {
	...
	while(cur_ptr < end_ptr) {
		...
		uint32_t token = fdt_u32_le2be((char *)cur_ptr); // 4 bytes one take
		cur_ptr += 4;
		...
		switch(token){
			...
			*/
				...
				cur_ptr += ut_align_up(ut_strlen((char*)cur_ptr),4);
				break;
			case FDT_END_NODE:
			/*
...
int fdt_traverse(fdt_callback cb,void * _dtb){
	...
	// dtb_ptr = (uintptr_t) 0x2eff7600;
	uart_send_string("\ndtb loading at: ");
	uart_send_hex(dtb_ptr); // display dtb_ptr hex
	uart_send_string("\r\n");
	...
	else uart_send_string("header magic check ok\n");
	/*
	+-----------------+
	| fdt_header      | <- dtb_ptr
	+-----------------+
...
void get_cpio_addr(int token,const char* name,const void* data,uint32_t size){
	UNUSED(size);
	if(token==FDT_PROP && ut_string_compare((char *)name,"linux,initrd-start")){
		cpio_addr = (char*)(uintptr_t)fdt_u32_le2be(data);
		uart_send_string("cpio address is at: ");
		uart_send_hex((uintptr_t)fdt_u32_le2be(data));
		uart_send_string("\r\n");
	}
}
