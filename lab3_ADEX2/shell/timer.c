...
void setTimeout(char *message,uint64_t seconds) {
	...
	if (!timer_head) {
		...
		unsigned int* address = (unsigned int*) CORE0_TIMER_IRQ_CTRL; // unmask timer interrupt
		*address = value;
	}
	
	create_timer(repeat_message,message_copy,seconds);
}
