...
void timer_irq_handler(char dontcare) {
	uart_send_string("in timer_irq_handler\n");
	//enable core_0_timer
...
void uart_transmit_handler(char dontcare) {
	...
	{
        // uart_send_string("in uart trans handler flag1 \n");
		...
		if (uart_write_head == uart_write_index) {
			uart_send_string("in uart trans handler flag2 \n");
			//uart_send_string("//////////////// \n");
			mmio_write(AUX_MU_IER, mmio_read(AUX_MU_IER) & ~0x2);
			if(uart_read_buffer[uart_read_index-1] == '\r' || uart_read_buffer[uart_read_index-1] == '\n')
			{
				...
				uart_write_index = 0;
				uart_write_head = 0;
				// uart_send_string("in uart trans handler flag4 \n");
			}	
		}	
	}
...
void uart_receive_handler(char data) {
	...
	create_task(uart_transmit_handler,2,'a'); // for echo
}


void irq_except_handler_c(char dontcare) {

	asm volatile("msr DAIFSet, 0xf"); // Disable interrupts
	// unsigned long long DAIFSet = 0;
	// asm volatile("mrs %0, DAIFSet":"=r"(DAIFSet));
	// uart_hex(DAIFSet);
	uart_send_string("in irq_except_handler_c \n");
	// uart_hex(uart_write_index);
	// uart_hex(uart_write_head);						  
	...
	if (core0_interrupt_source & CNTPSIRQ_BIT_POSITION) {
		...
		*address = 0;
		...
		create_task(timer_irq_handler,3,'a');
		// uart_send_string("out core0 interrupt\n");
    }

    // Handle UART interrupt
    if (irq_pending1 & AUXINIT_BIT_POSTION) {
		// uart_send_string("in irq pending \n");
    	// uart_hex(uart_write_index);
		// uart_hex(uart_write_head);
         if ((iir & 0x06) == 0x04) {
			char data = mmio_read(AUX_MU_IO) & 0xff;
			uart_send_string("in iir 0x04 aaaa \n");
			// Disable receive interrupt
			mmio_write(AUX_MU_IER, mmio_read(AUX_MU_IER) & ~(0x01));
			create_task(uart_receive_handler, 1, data);
		 }

		if ((iir & 0x06) == 0x02) {
			char data = 'a';
			uart_send_string("in iir 0x02 bbb \n");

			// Disable transmit interrupt
			mmio_write(AUX_MU_IER, mmio_read(AUX_MU_IER) & ~(0x02));
			create_task(uart_transmit_handler, 2, data);
		}
    }	
	...
	execute_tasks();
	//asm volatile("msr DAIFClr, 0xf"); // Enable interrupts
	// uart_send_string("out irq_except_handler_c\n");
...
void uart_irq_handler(){
    ...
    if ((iir & 0x06) == 0x02) {
        ...
        // Send data from the write buffer
        if (uart_write_head != uart_write_index) {
			/// uart_send_string("in uart_irq_handler_trans_!=\n");
            mmio_write(AUX_MU_IO, uart_write_buffer[uart_write_head++]);
            if (uart_write_index >= UART_BUFFER_SIZE) {
                uart_write_index = 0;
            }
        } else {
			/// uart_send_string("in uart_irq_handler_trans_else\n");
