...
#define BOOT_PGD_ATTR PD_TABLE // 01
...
void setup_identity_mapping()
{
    ...
    memset(IDENTITY_TT_L0, 0, 0x1000); // IDENTITY_TT_L0 is the address of the PGD
    memset(IDENTITY_TT_L1, 0, 0x1000); // IDENTITY_TT_L1 is the address of the PUD

    // set the entry of the PGD
    IDENTITY_TT_L0[0] = (pd_t)IDENTITY_TT_L1 | BOOT_PGD_ATTR;
    // set the two entry of the PUD
    ...
    unsigned long sctlr = read_sysreg(sctlr_el1); // sctlr_el1 --> System Control Register, EL1
    write_sysreg(sctlr_el1, sctlr | MMU_ENABLE);  // Enable the MMU by set the bit 1 of the sctlr_el1
}
