...
int set_couple_node(struct vnode **parent, struct vnode **child, const char *pathname, char *prefix)
{
    ...
    while (1)
    {
        ...
        {
            if (S_ISDIR(target_node->f_mode)) // S_ISDIR --> is directory
            {
                if (!_pathname)
                {
            ...
            else if (S_ISREG(target_node->f_mode)) // S_ISREG --> is normal file
            {
                *child = target_node;
                *parent = target_node->parent;
                return 0;
...
void init_rootfs()
{
    ...
    rootfs->fs = fs;
    rootfs->root = vnode_create("", S_IFDIR); // tag --> is directory
...
int vfs_open(const char *pathname, int flags, struct file **target)
{
    int res = 0;
    struct vnode *target_node = NULL;
    res = vfs_lookup(pathname, &target_node); // find child return 0, can't find child retrun -1

    // can't find file(vnode) and don't need creat file
    ...
    *target = kcalloc(sizeof(struct file));
    ...
    // can't find file(vnode) --> creat vnode
    ...
    if (!child)
    {
        ...
        (*target)->vnode = child; // file handle connect vnode
        return 0;
    }
...
// find child return 0, can't find child retrun -1
