...
struct boot_sector_t {
    ...
    uint8 sector_per_cluster; // Sectors Per Cluster
    uint16 reserved_sector_cnt; // Number of Reserved Sectors
    uint8 fat_cnt; // Number of FATs
    ...
    uint32 sector_cnt;
    uint32 sector_per_fat32; // Sectors Per FAT
    uint16 extflags;
    uint16 ver;
    uint32 root_cluster; // Root Directory First Cluster
    uint16 info;
    uint16 bkbooksec;
    uint8 reserved[12];
    uint8 drvnum;
...
struct dir_t {
    uint8 name[11];
    uint8 attr;
    uint8 ntres; // windows nt reserve
    uint8 crttimetenth; // create tiem tenth
    uint16 crttime; // creat time
    uint16 crtdate; // creat date
    uint16 lstaccdate; // last access data
    uint16 ch; // clster high
    uint16 wrttime; // write time
    uint16 wrtdate; // write date
    uint16 cl; // cluster low
...
struct long_dir_t {
    ...
    uint8 checksum;
    uint8 name2[12];
    uint16 fstcluslo; // first cluster
    uint8 name3[4];
} __attribute__((packed));
...
static int fat32fs_mount(struct filesystem *fs, struct mount *mount)
{
    ...
    fat->fat_lba = lba + fat->bs.reserved_sector_cnt; // fat_begin_lba
    fat->cluster_lba = fat->fat_lba +
                       fat->bs.fat_cnt * fat->bs.sector_per_fat32; // cluster_begin_lba
    ...
    list_for_each_entry(entry, head, list) {
        ...
        while (1) {
            ...
            if (idx >= 16) {
                ...
                sd_writeblock(lba, buf); // full use an cluster
    ...
    while (1) {
        ...
        for (int i = 0; i < DIR_PER_BLOCK; ++i) {
            ...
            if (lfn == 1) {
                ...
                lfn = 0;
                memset(&name, 0, sizeof(struct filename_t)); // clear name
...
static int fat32fs_write(struct file *file, const void *buf, size_t len)
{
    ...
    if (fat32fs_isdir(file->vnode)) { // is dir --> return 1
        return -1;
    }
...
static uint32 alloc_cluster(struct fat_info_t *fat, uint32 prev_cid)
{
    ...
    fat_lba = fat->fat_lba; // FAT location
    cid = 0;
