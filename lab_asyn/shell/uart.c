...
void uart_async_write(const char *buffer, int length) {
    uart_send_string("in uart_async_write\n");
    // uart_write_head = 1;
    // uart_read_index = 1;
    for (int i = 0; i < length; i++) 
    {
        // uart_send_string("qq\n");
        uart_write_buffer[uart_write_index++] = buffer[i];
        if (uart_write_head >= UART_BUFFER_SIZE) {
            uart_write_head = 0;
        }
    }
    // Trigger TX interrupt
    /// uart_send_string("Triiger TX interrupt\n");
    /// uart_hex(uart_write_index);
	/// uart_hex(uart_write_head);
    // unable transmit interrupt
    mmio_write(AUX_MU_IER, mmio_read(AUX_MU_IER) | (0x2));
    /// uart_send_string("flag1\n");
    /// uart_hex(uart_write_index);
	/// uart_hex(uart_write_head);
}

void uart_async_send(const char *str) {
    uart_send_string("in uart asy send ===========================\n");
